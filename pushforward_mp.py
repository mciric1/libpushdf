...
def loop1(a_var, d_ring, A_values, ideal_a, T):
    d_ring.order = MonomialOrderGRevLex
    z_vars = d_ring.variables()[d_ring.deriv_vars_end:]

    IDEAL_A_EV = [P.eval_coeffs(A_values, d_ring) for P in ideal_a]
    ...
    return tuple(my_mtx_to_np_mtx(derivative_companion_matrix(d_ring, t, z, a_var, G, B)) for z,t in zip(z_vars,T)), T

def loop1_mp(a_var, d_ring, A_values, ideal_a, T):
    TD,_ = loop1(a_var, d_ring, A_values, ideal_a, T)
    return TD

def loop2(I, J, FORM_EV, FORM_DEG, SYM, PRIME, T_Derivs, RA):
    factor = np.zeros_like(FORM_EV)

    for perm in SYM:
        if any(np_mtx_is_zero(T_Derivs[J[k]][I[perm(k+1)-1]]) for k in range(FORM_DEG)):
            pass
        else:
            p = np.eye(FORM_EV.shape[0], dtype=np.uint64)
            if perm.sign == -1:
                np.fill_diagonal(p, PRIME - 1)

            for k in range(FORM_DEG):
                p = np_mul_mod(p, T_Derivs[J[k]][I[perm(k+1)-1]], PRIME)

            factor += p

    return RA.coeff_ring(int(np.trace(np_mul_mod(FORM_EV, factor, PRIME))))
...
class PushforwardCalculator:
    ...
    def calc_pushforward_evaluated(self, A_values):
        ...
        # Compute the derivatives of Companion Matrices wrt first a-variable, needs to be done serially as the
        # Companion Matrices are also computed
        TD, T = loop1(self.a_var_names[0], self.D_RZ[0], A_values, self.IDEAL_A[0], None)

        # Compute the derivatives of Companion Matrices wrt the remaining a-variables in parallel
        mp_results = [self.pool.apply_async(loop1_mp, (a_var, self.D_RZ[i], A_values, self.IDEAL_A[i], T))
                      for i,a_var in enumerate(self.a_var_names[1:], 1)]
        T_Derivs = [TD] + [res.get() for res in mp_results]

        # Evaluate the differential form on the computed Companion Matrices
        FORM_EV = my_mtx_to_np_mtx(self.FORM(T))

        # Compute the evaluations of the rational prefactors of the pushforward differential form and return them for
        # rational reconstruction of the full form
        PRIME = self.RA.coeff_ring.p
        mp_results = [self.pool.apply_async(loop2, (I, J, FORM_EV, self.FORM_DEG, self.SYM, PRIME, T_Derivs, self.RA))
                      for I,J in self.PREFACTORS]
        return [res.get() for res in mp_results]
